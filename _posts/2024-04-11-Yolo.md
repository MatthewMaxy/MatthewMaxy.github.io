...
以前的工作大多采用 two-stage 的方法，几乎难以实现实现检测，而 Yolo 是一个可以实现端到端训练的目标检测新范式。
...
+ 每个网格还要预测一个类别信息，对应 $C$ 个类的准确度（文中 $C = 20$）
  + 类别概率是条件概率：$Pr(Classi \text{|}  Object)$
+ 在测试时，我们将条件类概率和单个框置信度预测相乘，这个分数体现了该类出现在框中的概率和预测框与对象的匹配程度，公式如下：
$$ Pr(Class_i \text{|} Object) * Pr(Object) * IOU^{truth}_{pred} = Pr(Class_i) * IOU^{truth}_{pred}$$
上述分析可以知道，最后输出纬度即为:
