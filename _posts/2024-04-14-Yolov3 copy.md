...
print(customer_response.content)
    因為顱骨的損傷，也耽擱了大量時間，雷鋒同志因工牺牲了。他生的偉大，死的光榮。在雷鋒的追悼會上有上萬人為雷鋒送行，他是人們心中永遠的英雄。
'''
```
...
此时如果直接采用2中的prompt方法，返回的结果在形式上可能与需求很相似，但我们会发现他的数据类型还是string
...
可以借用langchain的output解释器让输出转化为dict数据类型
...
Langchain会创建一个非常精准的 format instruction 描述：
![format instruction](/assets/Blogs/LangChain/1.png)

再对prompt模板进行拼接并进行query
